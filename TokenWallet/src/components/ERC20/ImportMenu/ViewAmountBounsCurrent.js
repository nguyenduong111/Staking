...
const ViewAmountBounsCurrent = ({ web3Token }) => {
    ...
    const onClickMint = async () => {
        ...
        try {
            ...
            let kq = await web3Token.methods.viewAmountBonusCurrent(data.arg1, data.arg2)
                                    .call();
            successMessage = `${kq}`;
        } catch (error) {
    ...
    return (
        <Grid container>
            <Grid item xs={12}>
                ...
                >
                    {data.loading ? <CircularProgress size={25} /> : "view amount bouns current (address user, uint256 ID)"}
                </Button>
            </Grid>
            <Grid item xs={12}>
                <TextField 
...
export default ViewAmountBounsCurrent